...
void main() {
  ...
  while (true) {
    ...
    int choice = InputHand.getInput<int>(
      massge: "Enter Your Choice: ",
      validator: InputHand.numberValidator(1, 4),
    );
    switch (choice) {
      case 1:
        theater.bookSeat();
...
class Theater {
  ...
  Theater({required this.rows, required this.cols}) {
    ...
    seats = List.generate(
      rows,
      (row) => List.generate(cols, (col) => Seat(row: row + 1, col: col + 1)),
    );
  }
  ...
  void bookSeat() {
    int row =
        InputHand.getInput<int>(
          massge: "Enter Your Row: ",
          validator: InputHand.numberValidator(1, rows),
        ) -
        1;
    int col =
        InputHand.getInput<int>(
          massge: "Enter Your Col: ",
          validator: InputHand.numberValidator(1, cols),
        ) -
        1;
    ...
    String name = InputHand.getInput<String>(
      validator: (input) => input.isNotEmpty ? input : null,
      massge: "Enter Your Name: ",
    );

    String phone = InputHand.getInput<String>(
      validator: (input) => input.isNotEmpty ? input : null,
      massge: "Enter Your Phone: ",
    );
...
class InputHand {
  ...
  static int? Function(String) numberValidator(int min, int max) {
    return (String input) {
